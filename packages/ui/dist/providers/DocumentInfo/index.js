  ...
  */
  ...
  const unlockDocument = useCallback(async (docID, slug_0) => {
    try {
      ...
      const query = isGlobal ? `where[globalSlug][equals]=${slug_0}` : `where[document.value][equals]=${docID}&where[document.relationTo][equals]=${slug_0}`;
  ...
  const updateDocumentEditor = useCallback(async (docID_0, slug_1, user) => {
    try {
      ...
      const query_0 = isGlobal_0 ? `where[globalSlug][equals]=${slug_1}` : `where[document.value][equals]=${docID_0}&where[document.relationTo][equals]=${slug_1}`;
      // Check if the document is already locked
      const request_0 = await requests.get(`${serverURL}${api}/payload-locked-documents?${query_0}`, {
        credentials: 'include'
      });
      const {
        docs: docs_0
      ...
      if (docs_0?.length > 0) {
        ...
        await requests.patch(`${serverURL}${api}/payload-locked-documents/${lockID_0}`, {
          body: JSON.stringify({
            user: userData
          }),
          credentials: 'include',
