...
export const init = function init() {
    this.payload.config.collections.forEach((collection)=>{
        ...
        if (collection.versions) {
            ...
            this.versions[collection.slug] = mongoose.model(versionModelName, versionSchema, versionCollectionName);
        }
        const modelName = getDBName({
            config: collection
        });
        const collectionName = this.autoPluralization === true && !collection.dbName ? undefined : modelName;
        this.collections[collection.slug] = mongoose.model(modelName, schema, collectionName);
    });
    ...
    this.payload.config.globals.forEach((global)=>{
        if (global.versions) {
            ...
            this.versions[global.slug] = mongoose.model(versionModelName, versionSchema, versionModelName);
        }
    });
};
